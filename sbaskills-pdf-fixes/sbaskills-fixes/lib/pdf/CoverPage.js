...
export function CoverPage({ title, subtitle, theme = brand }) {
  // Long titles need a smaller font so they wrap inside the page margins
  // instead of overflowing past the edge.
  const titleLength = (title || '').length;
  const titleFontSize = titleLength > 60 ? 18 : titleLength > 40 ? 22 : 26;

  const frontStyles = StyleSheet.create({
    coverTitle: { fontSize: titleFontSize, fontFamily: 'Helvetica-Bold', color: theme.primary, textAlign: 'center', marginTop: 200 },
  ...
  return (
    <Page size="A4" style={{ paddingHorizontal: 45, paddingVertical: 0, backgroundColor: theme.pageBackground || '#ffffff' }}>
