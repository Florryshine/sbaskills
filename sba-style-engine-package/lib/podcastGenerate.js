 ...
 * partway through.
 *
 * `style` selects the prompt from the Style Engine (lib/podcastStyles.js)
 * -- 'qa_conversation' (old default), 'deep_dive', 'classroom',
 * 'storytelling', 'tutorial', 'interview'. Unknown/legacy values (like the
 * old hardcoded 'teacher_examiner') fall back to qa_conversation, so
 * nothing that was already calling this breaks.
 */
 ...
 * same signature (plus the new `style` field, defaulted), same behavior
