...
// Handles: ## / ### headings, bullet lists, | table | rows |, **bold**, plain paragraphs.
...
export function parseMarkdownToBlocks(markdown) {
  ...
  while (i < lines.length) {
    ...
    const paraLines = [];
    while (i < lines.length && lines[i].trim() && !/^#{1,3}\s+/.test(lines[i]) && !lines[i].trim().startsWith('|') && !/^[-*]\s+/.test(lines[i].trim())) {
