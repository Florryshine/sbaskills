...
export function ReportCardDocument({ school, reportCard }) {
  ...
  const average = scores.length
    ? (scores.reduce((sum, s) => sum + Number(s.score || 0), 0) / scores.length).toFixed(1)
    : '-';
      ...
      <Page size="A4" style={styles.page}>
        ...
        <View style={styles.table}>
          <View style={styles.tableRow}>
            ...
            <Text style={[styles.tableHeaderCell, { flex: 2, borderRightWidth: 0 }]}>Remark</Text>
          </View>
          {scores.map((s, i) => (
            <View style={styles.tableRow} key={i}>
              ...
              <Text style={[styles.tableCell, { flex: 2, borderRightWidth: 0 }]}>{s.remark || '-'}</Text>
            </View>
          ))}
          <View style={styles.tableRow}>
