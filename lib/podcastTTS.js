...
// Effect files mapping (multiple options per effect for random selection)
...
/**
 * Synthesize a single line using openai-edge-tts with SSML.
 */
export async function synthesizeLine(text, speaker, emotion = 'neutral') {
  ...
  // Wrap text in SSML with prosody (speed + pitch) and a small pause
  const ssml = `<speak>
