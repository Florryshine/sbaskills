...
// Models routinely write multi-paragraph JSON string values (e.g. Facebook's
// "3-5 short paragraphs" caption prompt) using a literal line break between
// paragraphs instead of an escaped \n. That's invalid JSON — a raw \x0A/\x0D/\x09
// inside a string literal makes JSON.parse throw "Bad control character in
// string literal" — and removeControlChars() above intentionally leaves those
// bytes alone (they're valid outside strings, e.g. pretty-printed whitespace),
// so it never fixed this. This walks the text tracking whether we're inside a
// "..." string (respecting backslash-escapes) and only escapes raw
// newline/tab/CR when inString, leaving JSON structural whitespace untouched.
...
export function parseJsonFromText(text, expect = 'object') {
  ...
  for (const candidate of candidates) {
    const attempts = [
      ...
      escapeLiteralNewlinesInStrings(normalizeSmartQuotes(candidate)),
      removeControlChars(escapeLiteralNewlinesInStrings(normalizeSmartQuotes(candidate))),
      removeTrailingCommas(removeControlChars(escapeLiteralNewlinesInStrings(normalizeSmartQuotes(candidate)))),
    ];
