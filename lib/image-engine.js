...
async function renderWithSharpSvg(imageBuffer, title, category, preset) {
  ...
  // Embed the font as base64 so it renders even with zero system fonts on Vercel
...
export async function createBrandedThumbnail(imageBuffer, title, category, preset = IMAGE_PRESETS.hero) {
  try {
    ...
    // Log the FULL error (stack + message), not just .message — this is
    // what tells us whether it's a missing native binary, a load error,
    // or something else entirely. Check this in Vercel logs.
    console.error('❌ Canvas rendering failed, falling back to Sharp SVG. Full error:', canvasErr);
    try {
      // Fallback to Sharp SVG (now with embedded font)
