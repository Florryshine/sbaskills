...
// ── Video search — same providers, same keys, video endpoints instead of
// photo endpoints. Used by the quote-loop feature for a real-footage
// background instead of a static photo. Both APIs return several render
// sizes per clip; we grab something close to a 720-ish square-friendly
// size so the quote-loop recorder isn't cropping from a giant 4K master.
export async function searchPexelsVideoMulti(query, count = 3) {
  if (!PEXELS_API_KEY) return [];
  try {
    const url = `https://api.pexels.com/videos/search?query=${encodeURIComponent(query)}&orientation=portrait&per_page=${Math.max(count, 3)}`;
    const res = await fetch(url, { headers: { Authorization: PEXELS_API_KEY } });
    if (!res.ok) return [];
    const data = await res.json();
    return (data.videos || []).slice(0, count).map((video) => {
      // Pexels returns many render files per video — prefer something in
      // the ~720p range so the download stays small; fall back to the
      // smallest available file if nothing matches.
      const files = video.video_files || [];
      const preferred =
        files.find((f) => f.height && f.height <= 900 && f.height >= 480) ||
        files.sort((a, b) => (a.height || 9999) - (b.height || 9999))[0];
      return {
        url: preferred?.link,
    ...
    console.error('Pexels video search failed:', err.message);
    return [];
  }
}

export async function searchPixabayVideoMulti(query, count = 3) {
  if (!PIXABAY_API_KEY) return [];
  try {
    const url = `https://pixabay.com/api/videos/?key=${PIXABAY_API_KEY}&q=${encodeURIComponent(query)}&safesearch=true&per_page=${Math.max(count, 3)}`;
    const res = await fetch(url);
    if (!res.ok) return [];
    const data = await res.json();
    return (data.hits || []).slice(0, count).map((hit) => {
      // Pixabay buckets renders as tiny/small/medium/large — medium is
      // the closest match to Pexels' ~720p pick above.
      const rendition = hit.videos?.medium || hit.videos?.small || hit.videos?.large || hit.videos?.tiny;
      return {
        url: rendition?.url,
        previewUrl: hit.userImageURL,
        sourceUrl: hit.pageURL,
        photographer: hit.user,
    ...
    console.error('Pixabay video search failed:', err.message);
    return [];
  }
}
