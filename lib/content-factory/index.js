...
import { generateFounderPost } from './generators/founder';
import { attachCarouselMedia, queueVideoScript, attachHeroImage } from './media-attach';
...
const GENERATORS = {
  ...
  founder: generateFounderPost,
};

// Default platform set when the caller doesn't specify one — deliberately
// excludes 'founder'. A founder-voice post isn't appropriate for every
// topic (see generators/founder.js's anti-manufactured-inspiration rule),
// so it stays opt-in only, never bundled into "generate everything".
const DEFAULT_PLATFORMS = Object.keys(GENERATORS).filter((p) => p !== 'founder');
...
async function attachMediaIfNeeded(insertedRow, media) {
  ...
  try {
    ...
    } else if (media.type === 'hero_image') {
      await attachHeroImage(insertedRow.id, {
        title: insertedRow.title,
 ...
 * @param {Object} [options] - per-platform extra args, keyed by platform.
 *   Only the founder generator currently reads anything here:
 *   { founder: { voiceMode: 'founder'|'mentor'|'funny'|'reflective', founderContext: string } }
 *   Every other generator ignores its second argument, so this is safe to
 *   pass unconditionally.
 */
export async function runContentFactory(knowledgeAssetId, platforms = DEFAULT_PLATFORMS, options = {}) {
  ...
  await Promise.all(
    platforms.map(async (platform) => {
      ...
      try {
        const rows = await generator(asset, options[platform] || {}); // array of content_assets-shaped objects (+ optional _media)
...
export async function regenerateOne(contentAssetId) {
  ...
  // Founder posts store the real target platform in `platform` (so char
  // limits/UI filters work normally) and mark themselves via the
  // `founder_` asset_type prefix instead — so the generator to re-run has
  // to be resolved from asset_type first, falling back to platform for
  // every other row type.
  const generatorKey = existing.asset_type?.startsWith('founder_') ? 'founder' : existing.platform;
  const generator = GENERATORS[generatorKey];
  if (!generator) throw new Error(`No generator for: ${generatorKey}`);

  const asset = existing.knowledge_assets;
  // For founder posts, preserve the voice mode / context note that produced
  // the row being replaced, instead of silently falling back to defaults.
  const regenOptions = existing.metadata?.voiceMode
    ? { voiceMode: existing.metadata.voiceMode, founderContext: existing.metadata.founderContext }
    : {};
  const rows = await generator(asset, regenOptions);
