...
export default function StudentDashboard() {
  ...
  useEffect(() => {
    const getUser = async () => {
      ...
      setUser(user);
      // Fetch all published courses (replace with enrollments later)
      ...
      <div className="grid md:grid-cols-2 gap-6">
            ...
            <div className="space-y-4">
              {enrolledCourses.map(course => (
                <div key={course.id} className="border rounded p-4">
                  ...
                  <Link href={`/courses/${course.id}`} className="text-blue-600 text-sm mt-2 inline-block">
                    Continue Learning →
                  </Link>
                </div>
              ))}
        ...
        <div className="bg-white rounded-lg shadow p-6">
          ...
          <div className="mt-6 pt-4 border-t">
            <Link href="/courses" className="text-blue-600 hover:underline">Browse more courses →</Link>
          </div>
        </div>
      </div>
    </div>
