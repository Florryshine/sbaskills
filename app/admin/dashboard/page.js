...
export default function AdminDashboardPage() {
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-500">
              Monitor students, courses, revenue, and activity.
            </p>
          </div>
          <Link href="/admin/courses/new"
            className="hidden rounded-full bg-brand-yellow px-5 py-2.5 text-sm 
                       font-bold text-brand-dark hover:opacity-90 transition sm:block">
            + New Course
          </Link>
        </div>
      </section>
        ...
        {statCards.map((item) => (
          <Link key={item.label} href={item.link}
            ...
            <div className={`inline-flex h-10 w-10 items-center justify-center 
                            rounded-xl ${item.bg} text-xl mb-3`}>
              {item.icon}
            </div>
            ...
            <div className="mt-3 h-1.5 rounded-full bg-slate-100">
              <div className="h-1.5 w-1/2 rounded-full bg-brand-yellow 
      ...
      {/* Quick Actions */}
      <section className="rounded-2xl bg-white p-6 shadow-sm border border-slate-100">
        <h2 className="text-base font-extrabold text-brand-blue mb-4">
          Quick Actions
        </h2>
        <div className="flex flex-wrap gap-3">
          ...
          <Link href="/admin/students"
            className="rounded-full bg-brand-blue px-5 py-2.5 text-sm 
                       font-bold text-white hover:opacity-90 transition">
            👨‍🎓 View Students
          </Link>
          <Link href="/admin/courses"
            className="rounded-full border-2 border-brand-blue px-5 py-2.5 
                       text-sm font-bold text-brand-blue hover:bg-brand-blue 
                       hover:text-white transition">
            📚 Manage Courses
          </Link>
          <Link href="/"
            className="rounded-full border-2 border-slate-200 px-5 py-2.5 
                       text-sm font-bold text-slate-600 hover:border-slate-400 
                       transition">
            🌐 View Website
          </Link>
        </div>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <section className="rounded-2xl bg-white p-6 shadow-sm border border-slate-100">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-base font-extrabold text-brand-blue">
              Recent Students
            </h2>
            <Link href="/admin/students"
              ...
              {recentStudents.map((s) => (
                <li key={s.id}
                  ...
                  <div className="flex h-9 w-9 shrink-0 items-center justify-center 
                                  rounded-full bg-brand-blue text-white text-sm font-bold">
        ...
        <section className="rounded-2xl bg-white p-6 shadow-sm border border-slate-100">
          ...
          {recentCourses.length > 0 ? (
            <ul className="space-y-3">
              {recentCourses.map((c) => (
                <li key={c.id}
                  className="flex items-center gap-3 rounded-xl bg-slate-50 px-4 py-3">
                  <div className="flex h-9 w-9 shrink-0 items-center justify-center 
                                  rounded-full bg-purple-100 text-purple-600 text-lg">
                    📚
                  </div>
            ...
            <div className="rounded-xl bg-slate-50 py-8 text-center">
              ...
              <Link href="/admin/courses/new"
                className="mt-3 inline-block rounded-full bg-brand-yellow 
                           px-4 py-2 text-xs font-bold text-brand-dark">
                Create first course
              </Link>
            </div>
          )}
