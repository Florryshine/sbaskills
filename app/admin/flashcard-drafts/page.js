...
export default function FlashcardDraftsPage() {
  ...
  const deleteDraft = async (id) => {
    if (!confirm('Delete this flashcard draft?')) return;
    await supabase.from('flashcard_drafts').delete().eq('id', id);
    loadDrafts();
  };

  const updateStatus = async (id, status) => {
    await supabase.from('flashcard_drafts').update({ status }).eq('id', id);
    loadDrafts();
  };
  ...
  return (
    <div className="p-6">
      <h1 className="text-2xl font-bold text-brand-blue mb-6">🃏 Flashcard Drafts</h1>
      {loading ? <div className="text-center py-8">Loading...</div> : drafts.length === 0 ? (
        <div className="text-center py-8 text-gray-500">No flashcard drafts found.</div>
      ) : (
              ...
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-3">
                <div>
                  ...
                  <p className="text-sm text-gray-500">
                    Cards: {draft.cards?.length || 0} • Status: <span className={`font-semibold ${draft.status === 'published' ? 'text-green-600' : 'text-yellow-600'}`}>{draft.status}</span>
                  </p>
                </div>
                <div className="flex gap-2 flex-wrap">
                  <button onClick={() => toggleExpand(draft.id)} className="text-brand-blue px-4 py-2 rounded-xl text-sm font-bold hover:bg-blue-50">
                    {expandedId === draft.id ? 'Hide Cards' : 'Preview Cards'}
                  </button>
                  {draft.status === 'draft' ? (
              ...
              {expandedId === draft.id && (
                <div className="mt-4 max-h-96 overflow-y-auto bg-slate-50 p-4 rounded-xl">
                  {draft.cards?.map((card, idx) => (
                    <div key={idx} className="mb-3 border-b border-slate-200 pb-2 last:border-0">
                      <p className="font-semibold">Front: {card.front}</p>
                      <p className="text-sm">Back: {card.back}</p>
                      {card.explanation && <p className="text-xs text-gray-500">Explanation: {card.explanation}</p>}
                      {card.memory_trick && <p className="text-xs text-brand-blue">💡 {card.memory_trick}</p>}
                    </div>
                  ))}
                </div>
              )}
