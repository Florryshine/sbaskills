// app/api/admin/meme-loops/save/route.js
//
// Same shape and same "why runInBackground" reasoning as
// quote-loops/save — see that file's header for the full explanation of
// the Vercel-timeout fix this mirrors. Only the asset_type filter,
// bucket implied by the client upload, and created_by label differ.
...
export async function POST(request) {
  ...
  const { data: contentAsset, error: fetchError } = await supabase
    ...
    .eq('asset_type', 'meme_loop')
    .single();
  if (fetchError || !contentAsset) {
    return NextResponse.json({ error: 'Meme-loop content asset not found' }, { status: 404 });
  }
          ...
          for (const channel of channels) {
            if (videoPlatforms.includes(channel.platform)) {
              ...
              jobsToCreate.push({
                ...
                status: 'queued',
                created_by: 'meme-loop-auto-publish',
              });
            }
          }
