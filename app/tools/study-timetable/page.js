...
Generate a personalized weekly study timetable as a JSON object.
...
5. If exam is within 14 days, prioritize past questions and revision over new topics
6. Start sessions at realistic times (e.g. 7:00 AM or 8:00 AM)
7. Each study session should be 1-2 hours

Return ONLY valid JSON, no markdown, no explanation:
{
  "summary": "2 personalized sentences about the study strategy for this student",
  "weeklyPlan": {
    "Monday": [
      ...
      { "time": "9:45 AM - 11:15 AM", "subject": "Physics", "focus": "Motion and Forces", "type": "study" }
    ]
  },
...
Only include the available days. Be realistic, motivating, and specific to Nigerian exam content.`;
}
...
async function generateWithFallback(prompt) {
  ...
  for (const key of groqKeys) {
    try { return await callGroq(key, prompt); }
    catch (e) { errors.push(e.message); console.warn(e.message); }
  }
...
function exportPDF(timetable, subjects) {
  ...
  let html = `
  <html><head><title>Study Timetable – Shiney Brain Academy</title>
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .summary { background: #eff6ff; border-left: 4px solid #1e3a5f; padding: 12px 16px; margin-bottom: 24px; border-radius: 4px; line-height: 1.6; }
    h2 { color: #1e3a5f; font-size: 14px; margin: 20px 0 8px; border-bottom: 1px solid #e2e8f0; padding-bottom: 4px; }
    ...
    .tips h2 { border: none; margin-top: 0; }
  ...
  if (timetable.tips?.length) {
    html += `<div class="tips"><h2>💡 Study Tips from Your AI Coach</h2><ul>`;
    timetable.tips.forEach(t => { html += `<li>${t}</li>`; });
    html += `</ul></div>`;
  }

  html += `<div class="footer">Shiney Brain Academy · shineybrainacademy.com · Stay consistent, stay winning! 🌟</div>
  </body></html>`;
...
export default function StudyTimetablePage() {
  ...
  const handleReset = () => {
    setStep(1); setTimetable(null); setSubjects(['', '']);
    setWeakSubjects([]); setExamDate(''); setNotes('');
    setError(''); colorIdx = 0;
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50">
      ...
      <div className="bg-[#1e3a5f] text-white py-10 px-4 text-center">
        ...
        <p className="text-blue-200 text-sm mt-1">AI-powered · Personalized · Exam-ready</p>
      </div>
      ...
      <div className="max-w-2xl mx-auto px-4 py-8">
            ...
            {['Subjects', 'Schedule', 'Preferences'].map((label, i) => (
              <div key={i} className="flex items-center gap-2">
                <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold transition-all
                  ${step > i + 1 ? 'bg-green-500 text-white' : step === i + 1 ? 'bg-[#1e3a5f] text-white' : 'bg-gray-200 text-gray-500'}`}>
                  {step > i + 1 ? '✓' : i + 1}
                </div>
                <span className={`text-sm font-medium hidden sm:inline ${step === i + 1 ? 'text-[#1e3a5f]' : 'text-gray-400'}`}>{label}</span>
        ...
        {step === 1 && (
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
            <h2 className="text-xl font-bold text-[#1e3a5f] mb-1">What subjects are you studying?</h2>
            ...
            {validSubjects.length >= 2 && (
              <div className="mb-6">
                <p className="text-sm font-medium text-gray-700 mb-2">
                  Which are your weakest subjects? <span className="text-gray-400">(AI will give them more time)</span>
                </p>
            ...
            <button
              ...
              className="w-full bg-[#1e3a5f] text-white py-3 rounded-xl font-semibold disabled:opacity-40 hover:bg-[#16304f] transition-colors"
            >
              Next: Set Schedule →
            </button>
          </div>
        ...
        {step === 2 && (
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
            <h2 className="text-xl font-bold text-[#1e3a5f] mb-1">Set your study schedule</h2>
            <p className="text-gray-500 text-sm mb-5">Tell us when you're available</p>
            ...
            <div className="mb-6">
              <label className="block text-sm font-medium text-gray-700 mb-3">
                ...
                <span className="text-[#1e3a5f] font-bold text-base">{hoursPerDay} hrs</span>
              </label>
              <input
              ...
              {examDate && examDaysLeft !== null && (
                <p className={`text-xs mt-1 font-medium ${examDaysLeft < 14 ? 'text-red-500' : 'text-blue-600'}`}>
                  {examDaysLeft < 14
                    ...
                    : `📅 ${examDaysLeft} days until exam — AI will plan a full progressive study schedule.`}
                </p>
              )}
            </div>
            ...
            <div className="flex gap-3">
              ...
              <button
                ...
                className="flex-1 bg-[#1e3a5f] text-white py-3 rounded-xl font-semibold disabled:opacity-40 hover:bg-[#16304f] transition-colors"
              >
                Next: Preferences →
              </button>
            </div>
        ...
        {step === 3 && (
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
            <h2 className="text-xl font-bold text-[#1e3a5f] mb-1">Any special preferences?</h2>
            <p className="text-gray-500 text-sm mb-5">Help the AI personalize your plan even more</p>

            <textarea
              value={notes}
              onChange={e => setNotes(e.target.value)}
              placeholder="e.g. I prefer studying Maths in the morning. I have church on Sunday mornings. I want to focus on past questions for Chemistry. I struggle with organic chemistry..."
              rows={5}
            ...
            <div className="bg-blue-50 rounded-xl p-4 mb-5 text-sm">
              ...
              <ul className="space-y-1 text-blue-700">
                ...
                {examDate && <li>• <strong>Exam date:</strong> {new Date(examDate).toLocaleDateString('en-NG', { day: 'numeric', month: 'long', year: 'numeric' })} ({examDaysLeft} days away)</li>}
              </ul>
            </div>
            ...
            <div className="flex gap-3">
              ...
              <button
                ...
                disabled={loading}
                className="flex-1 bg-[#f0a500] text-white py-3 rounded-xl font-bold disabled:opacity-60 hover:bg-[#d4920a] transition-colors"
              >
                {loading ? (
        ...
        {step === 4 && timetable && (
          <div>
            {/* AI Summary */}
            {timetable.summary && (
              <div className="bg-[#1e3a5f] text-white rounded-2xl p-5 mb-5">
                <p className="text-xs font-semibold text-blue-300 uppercase tracking-wide mb-1">Your AI Study Strategy</p>
            ...
            {timetable.subjectHoursPerWeek && (
              <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 mb-5">
                <h3 className="font-bold text-[#1e3a5f] text-sm mb-3">Weekly Hours Per Subject</h3>
                <div className="flex flex-wrap gap-2">
            ...
            <div className="space-y-4 mb-5">
              {timetable.weeklyPlan && Object.entries(timetable.weeklyPlan).map(([day, sessions]) => (
                <div key={day} className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                  <div className="bg-[#1e3a5f] text-white px-5 py-3 flex justify-between items-center">
            ...
            <div className="flex gap-3 flex-wrap">
              <button
                ...
                className="flex-1 bg-[#1e3a5f] text-white py-3 rounded-xl font-semibold hover:bg-[#16304f] transition-colors flex items-center justify-center gap-2"
              >
                📄 Download as PDF
              </button>
              <button
