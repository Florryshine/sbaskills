...
export default function StudyTimetableClient() {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8">
      <div className="bg-white rounded-2xl shadow-sm border p-6">
        <h1 className="text-3xl font-extrabold text-brand-blue mb-2">📅 Study Timetable Generator</h1>
        <p className="text-gray-600 mb-6">Get a personalized study timetable from Mentor Florryshine based on your subjects and exam date.</p>
        ...
        <form onSubmit={generateTimetable} className="space-y-5">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <div>
              <label className="block text-sm font-semibold mb-1">Hours per day</label>
              <input
                type="number"
                min="1"
                max="12"
        ...
        {timetable && (
          <div className="mt-8 bg-brand-blue/5 rounded-2xl border border-brand-blue/20 p-6">
            <h2 className="text-xl font-extrabold text-brand-blue mb-4">📋 Your Personalized Study Timetable</h2>
            <div className="whitespace-pre-wrap text-gray-700 leading-relaxed prose max-w-none">{timetable}</div>
            <div className="mt-4 text-sm text-gray-500 border-t pt-4">💡 Powered by Mentor Florryshine</div>
          </div>
        )}
      </div>
    </div>
